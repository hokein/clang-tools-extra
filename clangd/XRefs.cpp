...
public:
  ...
  std::vector<DeclInfo> getFoundDecls() const {
    ...
    llvm::errs() << "Found declarations: \n";
    for (auto It : CanonicalDecls) {
      Result.emplace_back();
      It.first->dump(llvm::errs());
  ...
  handleDeclOccurence(const Decl *D, index::SymbolRoleSet Roles,
    ...
    if (Loc == SearchedLocation) {
      ...
      // Determine whether a given expression E is explicitly written at
      // SearchedLocation in the source code (as opposed to implicitly added by
      // the complier).
      auto isWrittenInCode = [&](const Expr *E) {
        assert(E && "E must be not null");
        if (E->child_begin() == E->child_end()) // no children
          return true;
        ...
        const auto *ImplicitCastE =
            dyn_cast<ImplicitCastExpr>(FirstChild);

        if (!ImplicitCastE) {
          // Temporary expressions are added by complier.
          bool isTemporaryExpression =
              isa<ExprWithCleanups>(FirstChild) ||
              isa<MaterializeTemporaryExpr>(FirstChild) ||
              isa<CXXBindTemporaryExpr>(FirstChild);
          return !isTemporaryExpression;
        }

        // Compiler will add implicit cast this expression for class member call
        // by default. ASTs are the same no matter whether "this" is written
        // in code ("this->method()" vs "method()"), so we still consider the
        // expression "method" is written.
        if (ImplicitCastE->isImplicitCXXThis())
          return true;
        // A written expression can be wrapped by an ImplicitCastExpr.
        // Check the underlying written expression is at SearchedLocation, true
        // means E is generated by the compiler (not written in code) and wraps
        // the written expression.
        const auto* SubWrittenExpr = ImplicitCastE->getSubExprAsWritten();
        bool isWrittenAtSearchedLoc =
            SubWrittenExpr->getBeginLoc() != SearchedLocation;
        return isWrittenAtSearchedLoc;
        //   llvm::errs() << "written expr\n";

        //   if (E) {
        //     llvm::errs() << "dump location!\n";
        //     E->dump(llvm::errs());
        //     auto range = E->getSourceRange();
        //     auto& SM = AST.getSourceManager();
        //     llvm::errs() << "dump search location!\n";
        //     Loc.dump(SM);
        //     llvm::errs() << "raw encoding: " << Loc.getRawEncoding() << "\n";
        //     //E->IgnoreImplicit()->dump(llvm::errs());
        //     //range.getBegin().dump(SM);
        //     //range.getEnd().dump(SM);
        //     //AST.getSourceManager().getSpellingLoc(test_loc).dump(AST.getSourceManager());
        //     //clang::index::printSymbolRoles(Roles, llvm::errs());
        //   }

        //   if (!E || E->child_begin() == E->child_end())
        //     return false;
        //   // Use the first child is good enough for most cases -- normally
        //   the
        //   // expression returned by handleDeclOccurence contains exactly one
        //   // child expression.
        //   const auto* FirstChild = *E->child_begin();
        //   const auto *ImplicitCastE =
        //   llvm::dyn_cast<ImplicitCastExpr>(FirstChild); if (!ImplicitCastE) {
        //     // implicit temporary expression
        ...
        //   }
        //   //FirstChild->ignoreParenBaseCasts();
        //   // Limitation here:
        //   // We can't distinguish with member call "this->func()" and
        //   "func()" from
        //   // the AST, as both ASTs are the same. When we see an implicit this
        //   cast
        //   // expr, we retreat E doesn't have implicit AST node.
        //   // ImplicitCXXThis
        //   if (ImplicitCastE->isImplicitCXXThis())
        //     return false;
        //   //
        //   // A written expression can be wrapped by an ImplicitCastExpr, if E
        //   // contains an ImplicitCastExpr whose written expression has the
        //   same
        //   // location of the SearchedLocation, we think E is
        //   // Heuristic to check an AST node corresponds to a written
        //   expression:
        //   // a written expression can be wrapped by an ImplicitCastExpr, we
        //   check
        //   // the innermost AST node which
        //   const auto* WrittenExpr = ImplicitCastE->getSubExprAsWritten();
        //   return WrittenExpr->getBeginLoc() == SearchedLocation;
        //   llvm::errs() << "written expr\n";
        //   // //FirstChild->isImp
        //   // if
        //   // Implici->getSubExprAsWritten()->dump(llvm::errs());
        //   // if (FirstChild->isImplicitCXXThis())
        //   //   return false;
        //   // if (FirstChild->getSubExprAsWritten()->getBeginLoc() ==
        //   SearchedLocation)
        //   //   return true;

        //   // return false;
        //   //FirstChild->getSubExprAsWritten()->getLo
        //   //if ()
        // //   return llvm::isa<ExprWithCleanups>(FirstChild) ||
        // //          llvm::isa<MaterializeTemporaryExpr>(FirstChild) ||
        // //          llvm::isa<CXXBindTemporaryExpr>(FirstChild) ||
      ...
      bool IsExplicit = !ASTNode.OrigE || isWrittenInCode(ASTNode.OrigE);
...
Optional<Hover> getHover(ParsedAST &AST, Position Pos) {
  ...
  SourceLocation SourceLocationBeg =
      getBeginningOfIdentifier(AST, Pos, SourceMgr.getMainFileID());
  SourceLocationBeg.dump(SourceMgr);
  // Identified symbols at a specific position.
  auto Symbols = getSymbolAtPosition(AST, SourceLocationBeg);
...
std::vector<Location> findReferences(ParsedAST &AST, Position Pos,
  ...
  llvm::errs() << "find ref at loc: \n";
  Loc.dump(SM);
  //Loc = SM.getFileLoc(Loc);
  //Loc.dump(SM);
  auto Symbols = getSymbolAtPosition(AST, Loc);
  llvm::errs() << "find symbols" << Symbols.Decls.size() << "\n";
  std::vector<const Decl *> TargetDecls;
  for (const DeclInfo &DI : Symbols.Decls) {
