...
class ParentStmtFinder : public RecursiveASTVisitor<ParentStmtFinder> {
public:
  ParentStmtFinder(const Stmt *Target) : Target(Target) {}

  // Returns the parent of Target stmt if exists.
  const Stmt *parent() const { return Parent; }

  bool VisitStmt(Stmt *S) {
    if (S == Target) {
      // Found the target stmt, stop visiting the tree.
      return false;
    }
    Parent = S;
    return true;
...
bool SymbolCollector::handleDeclOccurence(
  ...
  if (CollectRef &&
    ...
    // Ignore refs from implicit ASTNode generated by clang.
    if (ASTNode.OrigE && ASTNode.Parent) {
      ParentStmtFinder Finder(ASTNode.OrigE);
      if (ASTNode.Parent->hasBody()) {
        // OrigE is inside a function body.
        Finder.TraverseStmt(ASTNode.Parent->getBody());
      } else {
        // OrigE is not inside a function body, e.g. in global scope. We need
        // to traverse the declaration itself to find the parent of E.
        Finder.TraverseDecl(const_cast<Decl *>(ASTNode.Parent));
      }
      const Stmt *ParentStmt = Finder.parent();
      if (const auto *ICE = dyn_cast_or_null<ImplicitCastExpr>(ParentStmt)) {
        // clang could warp an implicitCastExpr around E even E is written.
        // If the origE is not written, it is generated by clang, ignore its
        // ref.
        if (ICE->getSubExprAsWritten() != ASTNode.OrigE)
          return true;
      }
    }
